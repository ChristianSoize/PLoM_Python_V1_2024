...
def mainWorkflow_Data_generation1():
    ...
    #  Subject      : allows to generate data for the Benchmark6_Direct_WithPartition_CondStat_pdf
    #
    #
    ...
    MatRqq_d = MatRxx_data[0:26,:]   # MatRqq_d(n_q,n_d)
    MatRww_d = MatRxx_data[26:28,:]  # MatRww_d(n_w,n_d)
    ...
    Indq_real = np.arange(1,27)                # loading the integers: 1,2, ...,26
    Indq_pos  = np.array([])

    #--- loading Indw_real(nwreal) and Indw_pos(nwpos)
    Indw_real = np.arange(1,3)                 # loading the integers: 1,2
    Indw_pos  = np.array([])

    #--- loading Indq_obs(nq_obs) and Indw_obs(nw_obs)
    Indq_obs = np.array([1,11,18,26])          # nq_obs component numbers 1,11,18,26 of QQ that are observed , 1 <= nq_obs <= n_q
    Indw_obs = np.array([1,2])                 # nw_obs component numbers 1,2 of WW that are observed,  1 <= nw_obs <= n_w
