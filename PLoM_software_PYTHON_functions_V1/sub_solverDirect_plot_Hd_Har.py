...
def sub_solverDirect_plot_Hd_Har(n_d, n_ar, MatReta_d, MatReta_ar, nbplotHsamples, nbplotHClouds, nbplotHpdf, nbplotHpdf2D, 
    ...
    if nbplotHsamples >= 1:
        for iplot in range(nbplotHsamples):
            ...
            numfig = numfig + 1
            plt.savefig(f'figure_SolverDirect_{numfig}_eta_ar_{ih}.png')
            plt.close()
    ...
    if nbplotHClouds >= 1:
        for iplot in range(nbplotHClouds):
            ...
            numfig += 1
            plt.savefig(f'figure_SolverDirect_{numfig}_eta_clouds_{ih}_{jh}_{kh}.png')
            plt.close()
    ...
    if nbplotHpdf >= 1:
        ...
        for iplot in range(nbplotHpdf):
            ...
            MIN    = min(np.min(Rd_ih), np.min(Rar_ih))
            MAX    = max(np.max(Rd_ih), np.max(Rar_ih)) 
            ...
            numfig = numfig + 1
            plt.savefig(f'figure_SolverDirect_{numfig}_pdf_H_d_H_ar{ih}.png')
            plt.close()
    ...
    if nbplotHpdf2D >= 1:
        ...
        for iplot in range(nbplotHpdf2D):
            ...
            numfig = numfig + 1
            plt.savefig(f'figure_SolverDirect_{numfig}_joint_pdf_Hd{ih}_Hd{jh}.png')
            plt.close()
    ...
    if nbplotHpdf2D > 0:
        ...
        for iplot in range(nbplotHpdf2D):
            ...
            numfig += 1
            plt.savefig(f'figure_SolverDirect_{numfig}_joint_pdf_Har{ih}_Har{jh}.png')
            plt.close()
