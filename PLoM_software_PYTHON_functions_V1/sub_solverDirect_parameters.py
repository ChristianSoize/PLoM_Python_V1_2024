...
def sub_solverDirect_parameters(nu,n_d,icorrectif,f0_ref,ind_f0,coeffDeltar,ind_constraints,iter_limit,alpha_relax1,iter_relax2,alpha_relax2):
    ...
    #  Subject      : calculating and loading the parameters used by solverDirect
    #
    #--- INPUT  
    ...
    #    An usual value for coeffDeltar is 20
    ...
    if ind_constraints == 0:     
        Ralpha_relax = np.array([])

    # There are constraints
    if ind_constraints >= 1:  
        Ralpha_relax = np.zeros(iter_limit)                                   
        if iter_relax2 == 1:
            Ralpha_relax[:] = alpha_relax2
        if iter_relax2 >= 2:
            for iter in range(1,iter_limit+1):
                if iter >= 1 and iter < iter_relax2:
                    Ralpha_relax[iter-1] = alpha_relax1 +(alpha_relax2 - alpha_relax1)*(iter-1)/(iter_relax2-1)
                if iter >= iter_relax2:
                    Ralpha_relax[iter-1] = alpha_relax2
    return s,sh,shss,f0,Deltar,M0estim,Ralpha_relax
