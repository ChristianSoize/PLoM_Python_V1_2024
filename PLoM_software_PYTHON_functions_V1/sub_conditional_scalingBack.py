...
def sub_conditional_scalingBack(nx_obs, n_x, n_ar, MatRx_obs, Indx_real, Indx_pos, Indx_obs, Rbeta_scale_real, Ralpha_scale_real,
    ...
    if np.any(Indx_obs < 1) or np.any(Indx_obs > n_x):
        raise ValueError('STOP4 in sub_scalingBack: at least one integer in Indx_obs is not within the valid range')  
                          # At least one integer in Indx_obs is not within the valid range.
    ...
    if nbpos >= 1:  # List_pos(nbpos_obs) contains the positive-valued observation numbers 
        ...
        for ipos in range(nbpos):
            ...
            for iobs in range(nx_obs):                     # iobs = 0,...,nx_obs-1
        ...
        if nbpos_obs < nbpos:
            List_pos             = List_pos[:nbpos_obs]              # List_pos(nbpos_obs), equivalent to matlab: List_pos(nbpos_obs+1:nbpos) = [];
    ...
    if nbreal >= 1:  # List_real(nbreal_obs) contains the real-valued observation numbers 
        ...
        for ireal in range(nbreal):
            ...
            for iobs in range(nx_obs):
                if jXreal == Indx_obs[iobs]:              # Indx_obs(nx_obs)
                    nbreal_obs += 1                       # nbpos_obs = nbpos_obs + 1
        ...
        if nbreal_obs < nbreal:
            List_real             = List_real[:nbreal_obs]                  # List_real(nbreal_obs)
    ...
    if nbreal_obs >= 1:
        MatRxx_obs[List_real - 1, :] = MatRxx_obs_real  # MatRxx_obs(nx_obs,n_ar),MatRxx_obs_real(nbreal_obs,n_ar),List_real(nbreal_obs)
    if nbpos_obs >= 1:
        MatRxx_obs[List_pos - 1, :] = MatRxx_obs_pos    # MatRxx_obs(nx_obs,n_ar),MatRxx_obs_pos(nbpos_obs,n_ar),List_pos(nbpos_obs)
