import numpy as np
from sub_solverDirect_Lrond_constraint2 import sub_solverDirect_Lrond_constraint2

def sub_solverDirect_Verlet_constraint2(nu, n_d, M0transient, Deltar, f0, MatReta_d, MatRa, MatRg, shss, sh, 
    ...
    #  Function name: sub_solverDirect_Verlet_constraint2
    #  Subject      : ind_constraints = 2 : constraints E{H_{ar,j}} = 0 for j = 1,...,nu  and E{H_{ar,j}^2} = 1 for j = 1,...,nu  
    ...
    #      Rlambda_iter(mhc)   = ( lambda_1,...,lambda_nu , lambda_{1+nu},...,lambda_{nu+nu} ), mhc = 2*nu
    #
    #--- OUTPUTS
    ...
    for k in range(M0transient):
        ...
        MatRLrond1s2 = sub_solverDirect_Lrond_constraint2(nu, n_d, MatReta_d, MatRa, MatRg, MatRZ1s2, shss, sh, Rlambda_iter)
