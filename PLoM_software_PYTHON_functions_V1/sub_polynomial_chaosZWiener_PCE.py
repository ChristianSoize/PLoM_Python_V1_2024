...
def sub_polynomial_chaosZWiener_PCE(K0, Ndeg, Ng, NnbMC0, MatRxiNg0):
    #---------------------------------------------------------------------------------------------------------------------------------------------
    #
    ...
    if Ndeg >= 1:
        ...
        MatRMM[0, :] = 1
        for k in range(K0 - 1):
            Rtemp = np.ones(NnbMC0)                                  # Rtemp(NnbMC0)
            for j in range(Ng):
        ...
        tolrel = 1e-12
        MatRFFtemp = MatRFF + tolrel * MaxD * np.eye(K0, K0)  # MatRFFtemp(K0,K0)
        # Cholesky decomposition
        try:
            MatRLLtemp = cholesky(MatRFFtemp, lower=True)  # MatRLLtemp(K0,K0) is a lower triangular matrix
            MatRLL = np.array(MatRLLtemp)  # MatRLL(K0,K0)
        except np.linalg.LinAlgError:
            # matrix not positive definite
            raise ValueError('STOP in sub_polynomial_chaosZWiener_PCE: The matrix MatRFF must be positive definite')
        MatRPsi0 = np.linalg.solve(MatRLL, MatRMM)  # MatRPsi0(K0,NnbMC0)
