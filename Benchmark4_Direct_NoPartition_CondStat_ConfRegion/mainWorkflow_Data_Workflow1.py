...
def mainWorkflow_Data_Workflow1():
    #----------------------------------------------------------------------------------------------------------------------------------------
    #
    #  Copyright: Christian Soize, Universite Gustave Eiffel, 11 Septembre 2024
    #
    ...
    #                 ind_workflow = 1 concerning Benchmark4_Direct_NoPartition_CondStat_ConfRegion
    ...
    if ind_step2 == 1:
        ...
        MatRplotHpdf2D = np.array([            # 2D array containing the 2 components numbers of H_ar for which the joint pdfs are plotted 
                                   ...
                                   [66, 67]    #    plot for the components 66, 67
                                   ])          #    nbplotHpdf2D = 2 
    ...
    if ind_step3 == 1:
        ...
        MatRplotClouds = np.array([                # 2D array of components of XX for which the clouds are plotted 
                                  [34,57,256],     #    plot the clouds for XX34,XX57,XX256  that is QQ34,QQ57,QQ256 
        ...
        MatRplotPDF = np.array([34,40,57,245,256]) # 1D array of components of XX for which the pdfs are plotted 
                                                   #    plot the pdf of XX37,XX40,XX57,XX245,XX256 that is
                                                   #    QQ37,QQ40,QQ57,QQ245,QQ256 and nbplotPDF = 5
    ...
    if ind_step4 == 1:
        ...
        if ind_step4_task13 == 1:
            ...
            MatRww0_obs = np.array([             # In this benchmark, n_w = 2
                                    [1    ,1.05], 
                                    [0.005,0.004] 
                                    ])  

                                             #--- For ind_pdf = 1:
                                             #    We consider XX34,XX40,XX57,XX245,XX256 that is QQ34,QQ40,QQ57,QQ245,QQ256, that is to say
                                             #    kk = 34,40,57,245,256, which correspond to indices k = 34,40,57,145,156 of kk = Indq_obs(k,1)
            Ind_Qcomp = np.array([34,40,57,245,256]) # 1D array Ind_Qcomp(nbQcomp): pdf of Q_obs_kk | WW_obs = Rww0_obs in which Q_obs_kk = MatRqq_obs(k,:). 
                                             ...
                                             # Indq_obs. Here, n_q = 400 and Indq_obs = np.vstack((np.arange(1, 101), np.arange(201, 301)))
                                             # that corresponds to the matlab array : Indq_obs = [(1:100)'     
                                             #                                                    (201:300)'];
